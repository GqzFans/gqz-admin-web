...
<template>
    <div class="page-content">
        ...
        <div class="page-addbtn-panel">
            <el-button type="primary" @click.native="addContent">添加腾讯视频资源</el-button>
        </div>
        <div>
            <el-table
                :data="tableData"
                ...
                <el-table-column
                    ...
                    width="120">
                    <template slot-scope="scope">
                        <el-button v-if="scope.row.videoStatus === '1'" style="color: #E6A23C;" type="text" size="small" @click="dropThis(scope.row)">
                            下架
                        </el-button>
                        <el-button v-if="scope.row.videoStatus === '2'" style="color: #67C23A;" type="text" size="small" @click="upThis(scope.row)">
                            上架
                        </el-button>
...
<script>
    export default {
        ...
        methods: {
            addContent() {
                this.$router.push({name: 'contentVideoUpload'});
            },
            ...
            requestTableData(postNum, postData) {
                ...
                _this.$http({
                    method: 'POST',
                    url: _this.$api.getHomeProjectLink() + '/api/gqz/content/video/queryGqzVideoList',
                    data: data
                }).then((res) => {
                    if (res.code === 200) {
            ...
            dropThis(row) {
                ...
                _this.$confirm('此操作将下架该视频，并无法展示, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    _this.$http({
                        method: 'POST',
                        url: _this.$api.getHomeProjectLink() + '/api/gqz/content/video/operateVideoById',
                        data: data
                    }).then((res) => {
            ...
            deleteThis(row) {
                ...
                _this.$confirm('此操作将永久删除该视频，并无法展示, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    _this.$http({
                        method: 'POST',
                        url: _this.$api.getHomeProjectLink() + '/api/gqz/content/video/deleteVideoById',
                        data: data
                    }).then((res) => {
                        if (res.code === 200 && res.result) {
                            _this.$message({
